...
from funcoes import soma_itens, acessar_carrossel, registar, listar_produtos, limpar_carrinho, atualizar_quantia, \
    ...
    acessar_inicial, deletar_usuario, produtos_para_envio, adicionar_endereco, acessar_enderecos, editar_endereco, \
    atualizar_senha, atualizar_nome, fechar_pedido
...
def renderizar_header(usuario):
    ...
    return {'logged_in': logged_in, 'total_items': soma, 'admin': is_admin, 'id_usuario': id_usuario_atual,
...
def gerenciar(usuario_id):
    ...
    if request.method == 'POST' and request.is_json:
        ...
        return jsonify({'status': 'ok', 'apelido': endereco.apelido, 'cep': endereco.cep, 'cidade': endereco.cidade,
            'estado': endereco.estado, 'rua': endereco.rua, 'numero': endereco.numero,
    ...
    # ---------- CASO 3: GET ----------
    return render_template('gerenciar_conta.html', senha_form=senha_form, nome_form=nome_form,
        enderecoform=enderecoform, enderecos=enderecos, **renderizar_header(current_user))
